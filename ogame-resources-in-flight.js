...
(function ()
{
	...
	if (document.location.href.indexOf ('/game/index.php?page=ingame&component=movement') >= 0)
	{
		...
		function doTable ()
		{
			...
			{
				let myFont = document.createElement ("font");
				myFont.color = color;
				let myTextNode = document.createTextNode (text);
				myFont.appendChild (myTextNode);
				return myFont;
			}
			function createCell (row, text, width, alignment, color)
			{
				let myTd = row.insertCell (-1);
			...
			{
				let fleetDetails = document.querySelectorAll (".fleetDetails");
		...
		function getResName (resName)
		{
			...
			let myLi = document.getElementById (resName + "_box");
			let title = myLi.title;
			if (title.length == 0)
			{
				let unsafe;
				try
				...
				let savedParam;
				let originalReloadResources = unsafe.reloadResources;
		...
		if (document.getElementById ("resourcesInFlight") != null)
			return;
		let resInfo = new Object;
		let directions = new Array ();
		let flightTypes = new Array ();
		let destinations = new Array ();
		let destinationNames = new Array ();
		let origins = new Array ();
		let originNames = new Array ();
		let myRes = new Array (0, 0, 0);
		let allFlights = document.querySelectorAll (".mission");
		for (var i = 0; i < allFlights.length; i++)
		{
			flightTypes [i] = allFlights [i].textContent;
			let matches = flightTypes [i].match (/\([^\)]+\)/);
			if (matches != null)
			{
		...
		for (var i = 0; i < allFlights.length; i++)
			...
			else
			{
				...
				{
					let mySpan = mySpans [mySpans.length - 2];
					let destName = (mySpan.title == "") ? mySpan.textContent.replace (/^\s+|\s+$/g, "") : mySpan.title.substring (mySpan.title.indexOf ("|") + 1);
					destinationNames [i] = ((onlyToCurrent) ? "=>" : "+") + destName.replace (/^\([^\)]+\) /, "");
				}
			}
		let flightCargo = new Object;
		allFlights = document.querySelectorAll (".fleetinfo");
		if (allFlights.length > 0)
		{
			for (var i = 0; i < allFlights.length; i++)
			{
				let trs = allFlights [i].getElementsByTagName ("tr");
				let any = false;
				let shipInfos = trs.length;
				...
				let type = directions [i] + flightTypes [i];
				if (resInfo [type])
				{
					for (var j = 0; j < 3; j++)
		...
		else
		{
			...
			for (var i = 0; i < allFlights.length; i++)
			{
				let trs = allFlights [i].textContent.split (/\n/);
				let any = false;
				let shipInfos = trs.length;
				if (trs [shipInfos - 1].id == "freeSpace")
					shipInfos--;
		...
		let metaTag = document.getElementsByName ("ogame-planet-name");
		let curPlanetName = (metaTag && metaTag.length > 0) ? metaTag [0].content : document.getElementById ("selectedPlanetName").textContent;
